/**
 * create_fire_detection_user.js
 * ─────────────────────────────────────────────────────────────
 * Creates a Firebase Auth user + Firestore user_profiles doc
 * with role = FIRE_DETECTION so they can access the
 * Fire Detection Command Center dashboard.
 *
 * Usage:
 *   node scripts/create_fire_detection_user.js
 *
 * Requirements:
...
const USER = {
  email:        'firedetection@safetymate.dev',   // change to your preferred email
  password:     'FireDetect@2024!',               // min 8 chars, must contain upper + number
  fullName:     'Fire Detection Technician',
  role:         'FIRE_DETECTION',
  phone:        '',                                // optional
...
async function main() {
  ...
  console.log(`    Role    : FIRE_DETECTION`)
  console.log(`    URL     : /detection/dashboard`)
  console.log('')
  process.exit(0)
}
