 ...
 * First page company users see after login.
 * Only shows the lockable platform modules (Fleet, Fire Ext, Fire Det).
 * Core safety modules are always accessible via the sidebar — no need to show them here.
 */
...
// ── Only the modules that can be locked/unlocked ──────────────────────────────
...
function ModuleCard({ module, isUnlocked, onOpen, onRequestAccess }) {
  ...
  if (isUnlocked) {
    return (
      <button
        type="button"
        className={`client-module-card client-module-card--${module.color}`}
        onClick={() => onOpen(module.to)}
        aria-label={`Open ${module.title}`}
      >
        <span className="client-module-card__bg-icon" aria-hidden="true">
          <BgIcon size={170} strokeWidth={1} />
        </span>
        ...
        <div className="client-module-card__footer">
          ...
          <span className="client-module-card__arrow" aria-hidden="true">
            <ArrowRight size={13} />
          </span>
        </div>
      </button>
  ...
  return (
    <div className={`client-module-card client-module-card--locked client-module-card--${module.color}`}>
      <span className="client-module-card__bg-icon client-module-card__bg-icon--locked" aria-hidden="true">
        <BgIcon size={170} strokeWidth={1} />
      </span>
      <div className="client-module-card__lock-badge" aria-label="Module not activated">
        <Lock size={12} />
      ...
      <div className="client-module-card__footer">
        <button
          type="button"
          className="client-module-card__request-btn"
          onClick={() => onRequestAccess(module)}
        >
          <Send size={12} />
          Request Access
        </button>
        ...
        <div className="client-modal client-modal--sm">
          <div style={{ textAlign: 'center', padding: '24px 20px 16px' }}>
            ...
            <p style={{ margin: '0 0 22px', fontSize: 13, color: 'rgba(148,163,184,0.8)', lineHeight: 1.55 }}>
              Your request for <strong style={{ color: '#4deba0' }}>{module.title}</strong> access
              has been sent to the SafetyMate administrator. You will be notified once access is granted.
            </p>
            <button type="button" className="client-btn client-btn--primary" onClick={onClose}>
              Done
            </button>
          </div>
        </div>
    ...
    <div className="client-modal-overlay" role="dialog" aria-modal="true">
      <div className="client-modal client-modal--sm">
        <div className="client-modal-header">
          ...
          <button type="button" className="client-drawer-close" onClick={onClose} aria-label="Close">
            <X size={16} />
          </button>
        </div>
        <div style={{ padding: '18px' }}>
          <p style={{ margin: '0 0 16px', fontSize: 13, color: 'rgba(148,163,184,0.85)', lineHeight: 1.55 }}>
            ...
            <strong style={{ color: 'rgba(235,242,255,0.95)' }}>{module.title}</strong>.
            An email will be sent to the SafetyMate administrator with your company details.
          </p>
          ...
          <textarea
            ...
            style={{
              ...
              color: 'rgba(235,242,255,0.9)', font: 'inherit', fontSize: 13,
              outline: 'none', boxSizing: 'border-box',
            }}
          />
          <div style={{ display: 'flex', gap: 10, marginTop: 16, justifyContent: 'flex-end', flexDirection: 'column' }}>
            {errorMsg && (
              <div style={{
                padding: '10px 14px', borderRadius: 8,
                background: 'rgba(255,83,95,0.1)', border: '1px solid rgba(255,83,95,0.3)',
                color: '#ff9aa2', fontSize: 12.5, fontWeight: 500,
              }}>
                ⚠ {errorMsg}
              </div>
            )}
...
export function ClientHomePage() {
  ...
  const { profile, organizationId, orgDoc, modules } = useAuth()
  ...
  return (
    <section className="client-page client-home-page">
      <header className="client-home-header">
        ...
        <p>
          Manage your subscribed platform modules below. Use the sidebar to access your core safety tools.
        </p>
      </header>
